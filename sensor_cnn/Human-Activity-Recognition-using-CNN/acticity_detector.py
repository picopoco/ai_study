...
#     #                    dtype={"user_id": int, "activity": object, 'timestamp': int, 'x-axis': int, 'y-axis': int,
#     #                           'z-axis': int})
#     data = pd.read_csv(file_path, header=None, names=column_names,low_memory=False).dropna()
...
#         cnt += 1
#
#     f = h5py.File("test_20.hdf5", "w")
#     label = f.create_dataset('label', shape=labels.shape, dtype=np.int)
#     seg = f.create_dataset('segment', shape=segments.shape, dtype=np.float64)
#     label[:] = np.array(a[:], dtype=np.int)
...
#
# dataset = read_data('./data/WISDM_at_v2.0_raw_r.txt')
